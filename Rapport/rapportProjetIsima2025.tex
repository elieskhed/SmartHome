...
\begin{document}
	...
	Pour assurer une gestion cohérente et efficace des données transmises par les maisons connectées au serveur central, il est nécessaire de mettre en place une structuration précises des \textbf{topics} et des \textbf{messages MQTT en utilisant le format JSON}. Cette structuration va ainsi permettre une identification claire des maisons, une organisation logique des équipements et un formats de données standarisé, disposé au sein de la base de données \textbf{InfluxDB}, facilitant leur stockage, leur exploitation, et l'extensibilité des données à ajouter. 
